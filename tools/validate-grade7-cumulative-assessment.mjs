...
const enginePath="grades/grade-07/assets/canonical-cumulative-assessment.js";
const engine=read(enginePath);
const navigator=read("assets/khaemenes-middle-learning-navigator.js");
const assessmentIndex=read("grades/grade-07/assessments/weekly-assessments.html");

expect(course?.weeks?.length===36,"Grade 7 cumulative assessment requires the canonical 36-week course.");
expect(course?.subjects?.length===9,"Grade 7 cumulative assessment requires nine canonical subjects.");
expect(Number(course?.course?.passingScore)===80,"Grade 7 cumulative assessment must preserve the 80% mastery threshold.");
expect(crosswalk?.weeks?.length===36,"Grade 7 cumulative assessment requires the canonical 36-week crosswalk.");
expect(Number(crosswalk?.masteryThreshold)===80,"Grade 7 crosswalk must preserve the 80% threshold.");

const midMarker=course.weeks.find(w=>/midyear|midterm/i.test(`${w.title||""} ${w.theme||""}`));
expect(Number(midMarker?.week)===17,`Canonical Grade 7 curriculum must place the Midyear Portfolio and Assessment at Week 17; found ${midMarker?.week??"none"}.`);
expect(/Midyear Portfolio and Assessment/i.test(midMarker?.title||""),"Grade 7 Week 17 must remain the explicit canonical midyear assessment marker.");
expect(Number(course?.assessments?.midterm?.itemCount)===120,"Grade 7 Midterm must preserve 120 items.");
expect(Number(course?.assessments?.final?.itemCount)===140,"Grade 7 Final must preserve 140 items.");
expect(Number(course?.assessments?.midterm?.passingScore)===80,"Grade 7 Midterm must preserve 80% passing.");
expect(Number(course?.assessments?.final?.passingScore)===80,"Grade 7 Final must preserve 80% passing.");
...
for(const token of [
  ...
  'KhaemenesGrade7Cumulative'
]) expect(engine.includes(token),`Canonical Grade 7 cumulative engine contract missing: ${token}`);
...
]) expect(!engine.includes(forbidden),`Grade 7 cumulative review/assessment must remain read-only; forbidden token: ${forbidden}`);
expect(engine.includes("localStorage.getItem(LEGACY_KEY)"),"Grade 7 cumulative review may read the legacy local record only as a compatibility fallback.");
expect(engine.includes('if(status?.status==="placement-mismatch")return blank()'),"Grade 7 placement mismatch must fail closed for score-aware review display.");
...
function pool(limit){
  ...
  for(const week of course.weeks.filter(w=>Number(w.week)<=limit)){
    ...
    for(const subject of course.subjects){
      ...
      expect(cell,`Missing Grade 7 cumulative source: Week ${pad(week.week)} ${subject.id}.`);
...
verifySelection("Grade 7 Midterm",17,120,96);
verifySelection("Grade 7 Final",36,140,112);

const shells=[
  ["grades/grade-07/assessments/midterm-review.html","midterm-review"],
  ["grades/grade-07/assessments/final-review.html","final-review"],
  ["grades/grade-07/assessments/midterm.html","midterm"],
  ["grades/grade-07/assessments/final-exam.html","final"]
];
for(const [file,mode] of shells){
  expect(exists(file),`Missing Grade 7 cumulative surface: ${file}`);
  ...
  expect(html.includes('../assets/canonical-cumulative-assessment.js'),`${file} must load the Grade 7 cumulative engine.`);
...
expect(!read("grades/grade-07/assessments/midterm.html").includes("Demonstrate Grade 7 A++ mastery in"),"Official Grade 7 Midterm must not retain the generic repeated legacy prompt.");
expect(!read("grades/grade-07/assessments/final-exam.html").includes("Demonstrate Grade 7 A++ mastery in"),"Official Grade 7 Final must not retain the generic repeated legacy prompt.");
...
for(const token of [
  ...
  '"assessments/midterm.html","Open Official Midterm"',
  '"assessments/final-exam.html","Open Official Final"'
]) expect(navigator.includes(token),`Learning Navigator Grade 7 cumulative route contract missing: ${token}`);

for(const token of ["midterm-review.html","Official Midterm","final-review.html","Official Final"]){
  expect(assessmentIndex.includes(token),`Grade 7 assessment directory cumulative route missing: ${token}`);
}

const grade8App=read("grades/grade-08/assets/app.js");
expect(!grade8App.includes("canonical-cumulative-assessment"),"Grade 8 must not inherit Grade 7 cumulative engine in this repair.");
