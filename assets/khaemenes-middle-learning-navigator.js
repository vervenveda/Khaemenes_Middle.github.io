(()=>{
  ...
  const packetHref=week=>grade==="06"?`evidence/weekly-evidence-packet.html?week=${pad(week)}`:grade==="07"?`evidence/weekly-evidence-packet.html?week=${pad(week)}`:`printables/week-${pad(week)}-packet.html`;
  const weeklyAssessmentHref=week=>grade==="06"?`evidence/weekly-mastery-check.html?week=${pad(week)}`:grade==="07"?`evidence/weekly-mastery-check.html?week=${pad(week)}`:`assessments/week-${pad(week)}-assessment.html`;
  ...
  function render(){
    ...
    DATA.weeks.forEach(w=>{
      ...
      actions.append(
        ...
        link(grade==="06"?"Weekly Evidence Packet":grade==="07"?"Weekly Evidence Packet":"Printable Packet",packetHref(w.week),"button"),
        link(grade==="06"?"Weekly Mastery Check":grade==="07"?"Weekly Mastery Check":"Weekly Assessment",weeklyAssessmentHref(w.week),"button")
      );
    ...
    reviews.append(
      buildReviewCard("Midyear Review Center",`Review Weeks 01–${pad(mid)} from the actual grade curriculum before opening the official Midterm.`,mid,"assessments/midterm.html","Open Official Midterm",state,hasCanonicalReviews?"assessments/midterm-review.html":null),
      buildReviewCard("Final Course Review",`Review all ${DATA.weeks.length} weeks, especially any recorded score below ${MIN}%, before opening the official Final.`,DATA.weeks.length,"assessments/final-exam.html","Open Official Final",state,hasCanonicalReviews?"assessments/final-review.html":null)
    );
