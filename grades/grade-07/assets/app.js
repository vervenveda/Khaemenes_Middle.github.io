(() => {
  ...
  function readState(){try{return JSON.parse(localStorage.getItem(KEY)) || {student:"Seventh Grade Scholar", weekly:{}, midterm:0, final:0, portfolio:false};}catch{return {student:"Seventh Grade Scholar", weekly:{}, midterm:0, final:0, portfolio:false};}}
  let state = readState();
  ...
  function bind(){
    $("saveProfile").addEventListener("click",()=>{state.student=$("studentName").value.trim()||"Seventh Grade Scholar";state.midterm=Math.max(0,Math.min(100,Number($("midtermScore").value||0)));state.final=Math.max(0,Math.min(100,Number($("finalScore").value||0)));state.portfolio=$("portfolio").checked;save();renderDashboard();renderWeeks();});
    $("clearRecords").addEventListener("click",()=>{if(!confirm("Clear local seventh-grade records on this device?"))return;localStorage.removeItem(KEY);state=readState();renderDashboard();renderWeeks();});
  }
  function exportRecords(){const payload={course:DATA.course.title,exported:new Date().toISOString(),state};const blob=new Blob([JSON.stringify(payload,null,2)],{type:"application/json"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="khaemenes-seventh-grade-records.json";a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}
