(() => {
  ...
  const KEY = "khaemenes_grade8_high_school_readiness_36_aplusplus_v1";
  const $ = id => document.getElementById(id);
  const esc = v => String(v ?? "").replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  function readState(){try{return JSON.parse(localStorage.getItem(KEY)) || {student:"Eighth Grade Scholar", weekly:{}, midterm:0, final:0, portfolio:false, transition:false};}catch{return {student:"Eighth Grade Scholar", weekly:{}, midterm:0, final:0, portfolio:false, transition:false};}}
  let state = readState();
  ...
  function bind(){
    $("saveProfile").addEventListener("click",()=>{state.student=$("studentName").value.trim()||"Eighth Grade Scholar";state.midterm=Math.max(0,Math.min(100,Number($("midtermScore").value||0)));state.final=Math.max(0,Math.min(100,Number($("finalScore").value||0)));state.portfolio=$("portfolio").checked;state.transition=$("transition").checked;save();renderDashboard();renderWeeks();});
    $("clearRecords").addEventListener("click",()=>{if(!confirm("Clear local eighth-grade records on this device?"))return;localStorage.removeItem(KEY);state=readState();renderDashboard();renderWeeks();});
  }
  function exportRecords(){const payload={course:DATA.course.title,exported:new Date().toISOString(),state};const blob=new Blob([JSON.stringify(payload,null,2)],{type:"application/json"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="khaemenes-eighth-grade-records.json";a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}
