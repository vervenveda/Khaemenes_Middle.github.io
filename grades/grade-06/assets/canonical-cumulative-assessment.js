(()=>{
  ...
  const CROSSWALK=window.KHAE_GRADE6_SUBJECT_WEEK_CROSSWALK;
  const CONT=window.KhaemenesGradeContinuity;
  const MODE=document.body?.dataset?.cumulativeMode||"midterm-review";
  const MIN=80;
  const LEGACY_KEY="khaemenes_grade6_middle_school_36_aplusplus_v1";
  ...
  function title(root,title,subtitle){
    const head=node("div","cum-head");head.append(node("p","kicker",isMidterm?"Grade 6 · Midyear":"Grade 6 · Final"),node("h1","",title),node("p","",subtitle));root.appendChild(head);
  }
  ...
  function reviewText(state){
    const lines=[`${isMidterm?"Grade 6 Midyear":"Grade 6 Final"} Review Guide`,`Coverage: Weeks 01–${pad(coverageEnd)}`,`Mastery standard: ${MIN}%`,""];
  ...
  function examText(items){
    const lines=[`${isMidterm?"Grade 6 Midterm":"Grade 6 Final"} · Canonical Cumulative Assessment`,`Coverage: Weeks 01–${pad(coverageEnd)}`,`Items: ${itemCount}; passing: ${passingItems}/${itemCount} (${MIN}% minimum).`,""];
  ...
  function actions(root,downloadLabel,downloadHandler,examHref){
    const row=node("div","cum-actions no-print");row.append(link("Back to Grade 6","../index.html","button"));
  ...
  function renderReview(root){
    ...
    title(root,isMidterm?"Midyear Review Center":"Final Course Review",isMidterm?`Refresh Weeks 01–${pad(coverageEnd)} before the official Midterm.`:`Refresh the full 36-week course before the official Final.`);
    identity(root);
    actions(root,"Download Review Guide",()=>downloadText(`grade-06-${isMidterm?"midyear":"final"}-review-guide.txt`,reviewText(state)),isMidterm?"midterm.html":"final-exam.html");
  ...
  function renderExam(root){
    ...
    title(root,isMidterm?"Sixth Grade Midyear Subject Demonstration":"Sixth Grade Final Readiness Demonstration",`Canonical cumulative assessment · Weeks 01–${pad(coverageEnd)} · ${itemCount} items · ${MIN}% minimum`);
    identity(root);
    actions(root,"Download Assessment Copy",()=>downloadText(`grade-06-${examType}-canonical-assessment.txt`,examText(items)),"");
    root.append(node("p","cum-note",`Official adult-reviewed cumulative assessment. Each item is worth one point. Passing requires at least ${passingItems}/${itemCount} items (${MIN}%). This page does not write a score to the learner record. After authorized review, record the official percentage in the Grade 6 learner record.`));
    ...
    for(const subject of DATA?.subjects||[]){
      ...
      const summary=node("summary");summary.append(node("span","",subject.title),node("span","cum-status",`${group.length} items`));details.appendChild(summary);
      const body=node("div","cum-subject-body");for(const item of group){const card=node("article","cum-item");card.append(node("h3","",`Item ${item.number}`),node("p","meta",`Week ${pad(item.week)} · ${item.focus}`),node("p","prompt",item.prompt),node("p","",`Evidence direction: ${item.evidence}`),node("div","cum-response"));body.appendChild(card)}details.appendChild(body);sections.appendChild(details);
    }
    root.appendChild(sections);
    const score=node("section","cum-score");score.append(node("h2","","Authorized Adult Review"),node("p","",`Record the official percentage only after reviewing all ${itemCount} items. The Grade 6 learner record remains the score authority.`));
    const grid=node("div","cum-score-grid");for(const label of [`Met: ____ / ${itemCount}`,`Required: ${passingItems} / ${itemCount}`,"Percent: ____ %","Adult reviewer: ____________________"]){grid.appendChild(node("div","",label))}score.appendChild(grid);root.appendChild(score);
  }

  styles();
  const mount=document.getElementById("canonicalCumulativeMount");
  if(!mount||!DATA||!CROSSWALK){if(mount)mount.textContent="The canonical Grade 6 cumulative curriculum data could not be loaded.";return}
  const shell=node("div","cum-shell");if(isReview)renderReview(shell);else renderExam(shell);mount.replaceChildren(shell);

  window.KhaemenesGrade6CumulativeAssessment=Object.freeze({version:VERSION,mode:MODE,coverageEnd,itemCount,passingItems,midyearBoundary,buildPool,selectEven,assessmentItems});
})();
