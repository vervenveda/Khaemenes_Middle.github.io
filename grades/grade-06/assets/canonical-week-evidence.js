(()=>{
  ...
  function packetText(){
    const lines=[`Khaemenes Academy Grade 6 · Week ${pad(weekNumber)} Weekly Evidence Packet`,row?.title||"",row?.essentialQuestion?`Essential question: ${row.essentialQuestion}`:"",""];
  ...
  function assessmentText(){
    const lines=[`Khaemenes Academy Grade 6 · Week ${pad(weekNumber)} Weekly Mastery Check`,row?.title||"",`Adult review: 11 domains × 2 points = ${TOTAL_POINTS}; passing ${PASSING_POINTS}/${TOTAL_POINTS} = 81.8% (meets ${MIN}% minimum).`,""];
  ...
  function renderPacket(root){
    ...
    controls(root,"Download Study Guide",()=>downloadText(`grade-06-week-${pad(weekNumber)}-evidence-guide.txt`,packetText()));
    root.append(node("p","canonical-evidence-note","This packet follows the canonical Grade 6 subject-by-week curriculum map. Use it beside the daily subject lessons to collect the evidence you will keep for adult review and your portfolio. It does not award mastery or change the learner record."));
  ...
  function renderAssessment(root){
    ...
    controls(root,"Download Review Copy",()=>downloadText(`grade-06-week-${pad(weekNumber)}-mastery-check.txt`,assessmentText()));
  ...
  if(!mount||!DATA||!CROSSWALK||!row||!map){
    if(mount)mount.textContent="This Grade 6 weekly evidence page could not load its canonical curriculum data.";
    return;
  }
  document.title=`Grade 6 Week ${pad(weekNumber)} · ${mode==="assessment"?"Weekly Mastery Check":"Weekly Evidence Packet"}`;
