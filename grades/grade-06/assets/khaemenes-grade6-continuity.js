/*
 * Khaemenes Grade 06 · Continuity Bridge v1.0.0
 * ------------------------------------------------
 ...
 * Grade 06 owns its course records; adult-reviewed evidence determines mastery.
 */
(function attachGrade6Continuity(global){
  "use strict";

  const LEGACY_KEY="khaemenes_grade6_middle_school_36_aplusplus_v1";
  const RECORDS_KEY="khaemenes_grade6_records_by_learner_v1";
  const ACTIVE_KEY="khaemenes_grade6_active_learner_v1";
  const MIGRATION_KEY="khaemenes_grade6_legacy_migration_claim_v1";
  ...
  function normalizeGrade(value){
    ...
    const match=raw.match(/(?:grade-?)?0?([6])\b/);
    return match?"grade-06":"";
  }
  ...
  function learner(){
    ...
    if(stage!=="middle"||grade!=="grade-06")return null;
    const learnerId=clean(raw.learnerId,120);
    if(!learnerId)return null;
    return Object.freeze({
      learnerId,
      familyId:clean(raw.familyId,120)||null,
      nickname:clean(raw.nickname||raw.displayName||"Sixth Grade Scholar",80),
      stage:"middle",
      grade:"grade-06",
  ...
  function resolveMentor(l=learner()){
    ...
    try{
      delegated=router?.delegate?.({
        ...
        surface:"khaemenes-middle-grade06",
    ...
    try{
      const legacyAssignment=router?.assignMentor?.({
        stage:"middle",
        ageBand:l.ageBand||undefined,
        interests:[...l.interests],
        surface:"khaemenes-middle-grade06",
      ...
      if(legacyAssignment?.status==="assigned"&&legacyAssignment?.mentor?.id==="archaemenes"){
        return Object.freeze({
          ...legacyAssignment.mentor,
          id:"archaemenes",
          name:"Archaemenes",
          presentationMode:legacyAssignment.mentor.presentationMode||PRESENTATION,
          providedBy:"Khaemenes Academy",
          delegatedBy:"NAIB"
        });
      }
  ...
  function normalizeState(value={}){
    ...
    return {
      student:clean(source.student||"Sixth Grade Scholar",80),
      weekly,
  ...
  function allRecords(){
    const value=readJSON(RECORDS_KEY,{});
  ...
  function migrateLegacyOnce(id){
    ...
    const claim=readJSON(MIGRATION_KEY,null);
    if(claim?.claimedBy&&claim.claimedBy!==id)return records;
    records[id]={
      ...normalizeState(legacy),
      learnerId:id,
      linkedAt:new Date().toISOString(),
      updatedAt:new Date().toISOString(),
      migration:{source:LEGACY_KEY,mode:"one-time-non-destructive-copy",migratedAt:new Date().toISOString()}
    };
    writeJSON(RECORDS_KEY,records);
    writeJSON(MIGRATION_KEY,{claimedBy:id,claimedAt:new Date().toISOString(),source:LEGACY_KEY});
    return records;
  }
  ...
  function loadState(){
    ...
    const records=migrateLegacyOnce(l.learnerId);
  ...
  function summary(){
    ...
    let next=1;
    for(let i=1;i<=36;i++){
      if(Number(state.weekly?.[i]||0)<80){next=i;break}
      next=36;
    }
    let reason="ok";
    if(!raw)reason="no-active-learner";
    else if(clean(raw.stage,40).toLowerCase()!=="middle")reason="stage-mismatch";
    else if(normalizeGrade(raw.grade||raw.gradeLevel)!=="grade-06")reason="grade-mismatch";
    return Object.freeze({
      connected:Boolean(global.KhaemenesFamilyRegistry),
      eligible:Boolean(l),reason,
      learner:l,
      mentor:l?resolveMentor(l):null,
      state,mastered,average,next,
      certificateReady:Boolean(l)&&mastered===36&&Number(state.midterm)>=80&&Number(state.final)>=80&&Boolean(state.portfolio)
    });
  }
  ...
  global.KhaemenesGrade6Continuity=Object.freeze({
    version:"1.0.0",
